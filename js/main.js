var profilesKey = 'watchlist_profiles';
...
(function($) {
    ...
    /// necessary 'cause we're abusing local storage to store JSON data    "haha, he said 'abusing'. also dude, did you really say "'cause" you only saved like 2 letters"
    ...
    jQuery(document).ready(function($) {
        ...
        $('#NG\\+ModalYes').click(function(event) {
            ...
            $('[id^="Watch_"], [id^="crow_"]').filter(':checked').each(function(){
    ...
    function initializeProfile(profile_name) {
        ...
        if (!('current_tab' in profiles[profilesKey][profile_name]))
            profiles[profilesKey][profile_name].current_tab = '#tabWatchList';
    ...
    // Setup ("bootstrap", haha) styling                 alot of 'haha' here , makes me think    ( ͡° ͜ʖ ͡°)
    function themeSetup(stylesheet) {
        if(stylesheet === null || stylesheet === undefined) { // if we didn't get a param, then
            stylesheet = $.jStorage.get("style") || "Superhero"; // fall back on "light" if cookie not set
        }
        $("#bootstrap").attr("href", themes[stylesheet]);
    }

    function buildThemeSelection() {
        var style = $.jStorage.get("style") || "Superhero";
    ...
    function calculateTotals() { //
        $('[id$="_overall_total"]').each(function(index) { //
            ...
            $('[id^="' + type + '_totals_"]').each(function(index) { //
                ...
                var regexFilter = new RegExp('^Watch_(.*)'); //
                ...
                if (checked === count) {
                    this.innerHTML = $('#' + type + '_list_totals_' + i)[0].innerHTML = 'DONE';
                    $(this).removeClass('in_progress').addClass('done');
                    $(this).parent('h3').addClass('completed');// Hide heading for completed category
                    $($('#' + type + '_list_totals_' + i)[0]).removeClass('in_progress').addClass('done');
                } else {
                    this.innerHTML = $('#' + type + '_list_totals_' + i)[0].innerHTML =  checked + '/' + count;
                    ...
                    $($('#' + type + '_list_totals_' + i)[0]).removeClass('done').addClass('in_progress');
                }
    ...
    $(function() {
        var jets = [new Jets({
            searchTag: '#Watch_search',
            contentTag: '#Watch_list ul'
        })];

        $('#Watch_search').keyup(function() {
            $('#Watch_list').unhighlight();
            $('#Watch_list').highlight($(this).val());
        });
    });
